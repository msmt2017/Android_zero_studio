...
public class TermuxTerminalSessionClient extends TermuxTerminalSessionClientBase {
    ...
    public void renameSession(final TerminalSession sessionToRename) {
        if (sessionToRename == null)
            return;
        TextInputDialogUtils.textInput(mActivity, R.string.title_rename_session, sessionToRename.mSessionName, R.string.action_rename_session_confirm, new TextInputDialogUtils.TextSetListener(){
    ...
    public void checkAndScrollToSession(TerminalSession session) {
        ...
        termuxSessionsListView.postDelayed(new Runnable() {
            ...
            if (colorsFile.isFile()) {
                {
                    InputStream in = null;
                    try {
                        in = new FileInputStream(colorsFile);
                        props.load(in);
                    } finally {
                        if (in != null) {
                            in.close();
