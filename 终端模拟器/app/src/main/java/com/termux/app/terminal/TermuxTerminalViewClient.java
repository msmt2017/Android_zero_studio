...
public class TermuxTerminalViewClient extends TermuxTerminalViewClientBase {
    ...
    private Runnable getShowSoftKeyboardRunnable() {
        if (mShowSoftKeyboardRunnable == null) {
            mShowSoftKeyboardRunnable = new Runnable() {
    ...
    public void showUrlSelection() {
        ...
        final AlertDialog dialog = new AlertDialog.Builder(mActivity).setItems(urls, new OnClickListener() {
    ...
    public void reportIssueFromTranscript() {
        ...
        if (transcriptText == null)
            return;
        MessageDialogUtils.showMessage(mActivity, TermuxConstants.TERMUX_APP_NAME + " Report Issue", mActivity.getString(R.string.msg_add_termux_debug_info), mActivity.getString(R.string.action_yes), new OnClickListener() {

            @Override
            public void onClick(DialogInterface dialog, int which) {
                reportIssueFromTranscript(transcriptText, true);
            }
        }, mActivity.getString(R.string.action_no), new OnClickListener() {

            @Override
            public void onClick(DialogInterface dialog, int which) {
                reportIssueFromTranscript(transcriptText, false);
            }
        }, null);
    }

    private void reportIssueFromTranscript(final String transcriptText, final boolean addTermuxDebugInfo) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                ReportActivity.startReportActivity(mActivity, new ReportInfo(userActionName, TermuxConstants.TERMUX_APP.TERMUX_ACTIVITY_NAME, title, null, reportString.toString(), "\n\n" + TermuxUtils.getReportIssueMarkdownString(mActivity), false, userActionName, Environment.getExternalStorageDirectory() + "/" + FileUtils.sanitizeFileName(TermuxConstants.TERMUX_APP_NAME + "-" + userActionName + ".log", true, true)));
            }
        }.start();
    }
