...
public final class TermuxService extends Service implements TermuxTask.TermuxTaskClient, TermuxSession.TermuxSessionClient {
	...
	static TermuxService termuxService;
    /**
    ...
    private synchronized void killAllTermuxExecutionCommands() {
        ...
        List<TermuxSession> termuxSessions = new ArrayList<TermuxSession>(mTermuxSessions);
        ...
        List<TermuxTask> termuxTasks = new ArrayList<TermuxTask>(mTermuxTasks);
        ...
        List<ExecutionCommand> pendingPluginExecutionCommands = new ArrayList<ExecutionCommand>(mPendingPluginExecutionCommands);
            ...
            public void run() {
                if (termuxTask != null) {
                    ...
                    if (executionCommand != null && executionCommand.isPluginExecutionCommand)
                        PluginUtils.processPluginExecutionCommandResult(TermuxService.this, LOG_TAG, executionCommand);
    ...
    public synchronized void setTermuxTerminalSessionClient(TermuxTerminalSessionClient termuxTerminalSessionClient) {
        ...
        for (int i = 0; i < mTermuxSessions.size(); i++) mTermuxSessions.get(i).getTerminalSession().updateTerminalSessionClient(mTermuxTerminalSessionClient);
    }
    ...
    public synchronized void unsetTermuxTerminalSessionClient() {
        for (int i = 0; i < mTermuxSessions.size(); i++) mTermuxSessions.get(i).getTerminalSession().updateTerminalSessionClient(mTermuxTerminalSessionClientBase);
